classdef CapSensor < JavaDevice
    ...
    properties (Access = protected)
        ...
        clock
        cJavaName           = 'RSA';               % Data Translation Unit 1
        u8JavaIndex         = uint8([0, 1]);        % The indexes of the implemented Java VoltMeter class that will be used
        cecUIDispName       = {'Cap 1', 'Cap 2'};      % (cell of char) name of each axes for display and and clock purposes
        cecUIType           = {'ho', 'ho'};  
    ...
    methods
        ...
        function this = CapSensor( ...
            ...
            this.cJarPath           = fullfile(pwd, 'CapSensor.jar');
            this.cPackage           = 'cxro.common.device';
            this.cConstructFcn      = sprintf('CapSensor(''%s'',''iman.lbl.gov'')', this.cJavaName);
            
            % Singular Java properties
            this.cConstructFcn2     = sprintf('getCapSensor(''%s'')', this.cJavaName);
            ...
            for k = 1:length(this.cecUIDispName)
                ...
                this.cex{k}.api = APICapSensor(this, this.u8JavaIndex(k));
